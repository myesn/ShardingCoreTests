...
namespace ShardingCoreTests.MultiField
{
    public class OrderVirtualRoute : AbstractSimpleShardingMonthKeyDateTimeVirtualTableRoute<Order>
    {
        ...
        {
            builder.ShardingProperty(x => x.CreateTime);
            builder.ShardingExtraProperty(x => x.OrderNo);
        }
        ...
        public override Func<string, bool> GetExtraRouteFilter(object shardingKey, ShardingOperatorEnum shardingOperator, string shardingPropertyName)
        {
            ...
            {
                case nameof(Order.OrderNo): return GetOrderNoRouteFilter(shardingKey, shardingOperator).Compile();
        ...
        /// <param name="shardingOperator"></param>
        /// <returns></returns>
        private Expression<Func<string, bool>> GetOrderNoRouteFilter(object shardingKey, ShardingOperatorEnum shardingOperator)
        {
            //将分表字段转成订单编号
